...
"""Start the existing recipe with an explicit, private context observer.
...
from dspark_context_snapshot import ENV, file_hash, require, write_json

DEFAULT_STATE = Path("/home/tyj/glm52-ms1")


def prepare(state, scripts):
    require(ENV not in os.environ, "Nested snapshot launch is not supported")
    ...
    run = state / "evidence" / f"context-snapshot-{run_id}"
    boot = run / "bootstrap"
    boot.mkdir(parents=True)
    observer = scripts / "dspark_context_snapshot.py"
    (boot / observer.name).write_bytes(observer.read_bytes())
    (boot / "sitecustomize.py").write_text(
        "from dspark_context_snapshot import install\ninstall()\n"
    )
    ...
    config = {
        "run_dir": str(run),
        "run_id": run_id,
        "rid": "ms1-context-" + uuid.uuid4().hex,
        "cache_salt": "ms1-context-cache-" + uuid.uuid4().hex,
        "max_bytes": 1024**3,
        "repo": str(repo),
        "git_head": head,
        "observer_sha256": file_hash(observer),
        "client_sha256": file_hash(scripts / "probe_context_snapshot.py"),
        "reference_sha256": file_hash(scripts / "dspark_local_reference.py"),
        "git_status": subprocess.check_output(
            ["git", "status", "--porcelain=v1"], cwd=repo, text=True
        ),
        "launcher_sha256": file_hash(scripts / "with_context_snapshot.py"),
        "scope": "one cold first prefill, rank0, first/middle/last rows, five dense layers",
    }
    write_json(run / "config.json", config)
    write_json(
        state / "context-snapshot-current.json", {"config": str(run / "config.json")}
    )
    env = os.environ.copy()
...
def main():
    ...
    print(f"Context snapshot evidence: {config['run_dir']}", flush=True)
    print(
        "Start original static/eager recipe; use probe_context_snapshot.py in a second terminal when ready.",
        flush=True,
    )
